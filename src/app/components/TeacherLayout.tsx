...
export function TeacherLayout({ children }: TeacherLayoutProps) {
  ...
  const menuItems = [
    ...
    { icon: FileText, label: 'New Incident', path: '/teacher/incidents/new' },
    { icon: FileText, label: 'My Incidents', path: '/teacher/incidents' },
    { icon: Bell, label: 'Notifications', path: '/teacher/notifications' },
    { icon: Settings, label: 'Settings', path: '/teacher/account' },
  ];
        ...
        <div className="h-full flex flex-col">
          ...
          <div className="p-4 border-b border-white/15 flex-shrink-0">
            <div className={`flex items-center px-4 py-3 ${!sidebarOpen && 'justify-center'}`}>
              ...
              {sidebarOpen && (
                <div className="ml-3">
                  <div className="text-sm font-medium text-white">Maria Johnson</div>
                  <div className="text-xs text-white/60">Teacher</div>
                </div>
              )}
            </div>
          </div>
      ...
      <div className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ${
        ...
        <header className="md:hidden bg-brand px-4 py-3 flex items-center justify-between sticky top-0 z-30">
          ...
          <Avatar className="w-8 h-8 bg-white/20">
            <AvatarFallback className="text-white text-sm">MJ</AvatarFallback>
          </Avatar>
        </header>
